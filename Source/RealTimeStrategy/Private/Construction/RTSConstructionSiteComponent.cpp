...
void URTSConstructionSiteComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	// Update health.
	URTSCombatComponent* CombatComponent = GetOwner()->FindComponentByClass<URTSCombatComponent>();

	if (IsValid(CombatComponent))
	{
		const float MaximumHealth = CombatComponent->GetMaximumHealth();
		const float HealthIncrement = MaximumHealth * (1 - InitialHealthPercentage) * ConstructionProgress / ConstructionTime;

		URTSGEConstruction* Effect = NewObject<URTSGEConstruction>();
		Effect->Update(CombatComponent, HealthIncrement);
...
void URTSConstructionSiteComponent::StartConstruction()
{
	...
	{
		const float MaximumHealth = CombatComponent->GetMaximumHealth();
		const float HealthIncrement = MaximumHealth * (1 - InitialHealthPercentage);
